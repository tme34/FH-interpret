...
server <- function(input, output, session) {
  ...
  interpretText<- reactive({
    ...
    if ((ac_prox==0 | ta_prox==0) & (het_prox=="0" | n_prox==0)) {
      ...
      gnomad <- glue::glue(paste("{hgvs} is found in {ac} out of {ta} alleles in <a href='https://gnomad.broadinstitute.org/variant/",gnomad_var,"?dataset=gnomad_r4' target='_blank'>","gnomAD</a>, but not amonmg ~470.000 participants in UK Biobank.", sep = ""))
      ukb <- ""
    } else if ((ac_prox==0 | ta_prox==0) & (het_prox!="0" & n_prox!=0)){
